...
Write-Host "`nCreating a directory: C:\Backup_job_report. This operation will override if you have a directory with the same name.`n"
New-Item C:\Backup_job_report -Type Directory -Force | Out-Null

$logFile = "C:\Backup_job_report\AzureBackupReport.log"
If(Test-Path "$logFile")
{
    ...
    {
        Remove-Item $logFile
        New-Item -Path "C:\Backup_job_report" -Name "AzureBackupReport.log" -ItemType file | Out-Null
...
Else
{
    $logFile = New-Item -Path "C:\Backup_job_report" -Name "AzureBackupReport.log" -ItemType file
...
foreach($Tag in $TagList) {
    ...
    $ConsolidatedInfo | Select-Object -Property WorkloadName,JobID,BackupManagementType,StartTime,EndTime,@{label= "Duration";Expression={ $sec = [math]::floor(($_.duration -split ":")[2]);
        $hour = ($_.duration -split ":" )[0] ; $min = ($_.duration -split ":")[1] ; "$($hour):$($min):$($sec)"}},Status | Export-Excel -WorkSheetname ConsolidatedReport -Path "C:\Backup_job_report\Backup_Report.xlsx"
...
"$FormattedDate : INFO : The file C:\Backup_job_report\Backup_Report_$F-to-$T.xlsx has been created successfully and is ready to use." | Out-File -FilePath $logfile -Append
Echo "#---------------------------------------------------------------------------------#"
